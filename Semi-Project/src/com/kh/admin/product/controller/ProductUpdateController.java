...
public class ProductUpdateController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			BookMaster bm = new BookMaster(bookNo, bookCode, bookTitle, bookHead, bookContent, authorInfo,
					price, author, title, stock, pblctDate, publisher);
			
			// 4) BookTag VO 객체로 가공
			BookTag tc1 = new BookTag(1, tagContent1, bookNo);
			BookTag tc2 = new BookTag(2, tagContent2, bookNo);
			...
			int result = new ProductService().updateProduct(bm, at, tc1, tc2);
